...
@injectable()
export class InvestorRoutes extends BaseRoute{
    constructor(
        @inject(Tokens.authController) private authController : UserController
    ){super()}
    protected initRoute(): void {
        this.router
        .post("/signup",attachRole(Roles.investor_role),(req:CustomRequest,res,next) => this.authController.register(req,res,next))
        .post('/otp/verifyOtp',attachRole(Roles.investor_role),(req:CustomRequest,res,next) => this.authController.verifyOtp(req,res,next))
